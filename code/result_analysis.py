...
class result_analysis(object):
    ...
    def evaluate_results(
        ...
        if check_question_action and \
            test_question is not None and test_action is not None:
            assert(len(Y_pred) == len(test_question))
            print("Test sample number: " + str(len(Y_pred)))
            print("Predicted positive test samples' question and action probability:")
            for i in range(len(Y_pred)):
                if Y_pred[i]:
                    print(test_question[i], test_action[i])
            print("Predicted negative test samples' question and action probability:")
            for i in range(len(Y_pred)):
                if not Y_pred[i]:
                    print(test_question[i], test_action[i])
            pos_num, neg_num = 0, 0
            print("Positive test samples' question and action probability:")
            for i in range(len(test_Y)):
                if test_Y[i]:
                    pos_num += 1
                    print(test_question[i], test_action[i])
            print("Negative test samples' question and action probability:")
            for i in range(len(test_Y)):
                if not test_Y[i]:
                    neg_num += 1
                    print(test_question[i], test_action[i])
            print("Test samples' question and action probability:")
            for i in range(len(test_Y)):
                print(test_question[i], test_action[i])
            print("Positive number: " + str(pos_num))
            print("Negative number:" + str(neg_num))
