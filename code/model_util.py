...
class model_util(object):
    ...
    def simple_arci(
        ...
        mlp1 = Dense(dense_size, activation='relu')(input1)
        mlp2 = Dense(dense_size, activation='relu')(input2)
        
        concatenated_tensor = Concatenate(axis=1)([mlp1, mlp2])
        ...
        print('Successfully build the simple arci model!')
        return model
    ...
    def baseline_predict(
        ...
        for i in range(len(test12)):
            val12 = test12[i][0] if not model_name.endswith('solo') else 1.0
            val23 = test23[i][0] if not model_name.endswith('solo') else 1.0
            qa = (test_question[i] + test_action[i]) / 2
            if model_name.endswith('max'):
                qa = max(test_question[i], test_action[i])
            elif 'question' in model_name:
                qa = min(test_question[i] * 2, 1.0)
            elif 'action' in model_name:
                qa = test_action[i]
            # The rule based baseline method
            val12 = 1 if val12 > 0.5 else 0
            val23 = 1 if val23 > 0.5 else 0
            qa = 1 if qa > 0.95 else 0
            combine_y_p.append(val12 * val23 * qa)
        y_p = [
            [val, 1.0 - val]
            for val in combine_y_p
        ]
